...
function crearDirectorioItcss() {
   let path = "./src/assets/";

   //CARPETAS DE IMAGENES
   console.log("CREANDO CARPETAS DE IMAGENES...");
   crearCarpetas(path + "img{jpg,png,svg}");
...
function crearDirectorioBootstrap() {
   let path = "./src/assets/";

   // CREAR CARPETA DE IMAGENES
   crearCarpetas(path + "images{jpg,png,svg}");
   // CREAR CARPETAS Y ARCHIVOS DE JAVASCRIPT
   crearCarpetas(path + "js");
   crearArchivos(path + "js/index.js");
   // CREAR CAREPTAS Y ARCHIVOS DE PUG
   crearCarpetas("./src/pug{pages,tools}");
   crearCarpetas("./src/pug/tools{includes,layout,mixins,templates,variables}}");
   crearArchivos("./src/pug/index.pug")
}

crearDirectorioItcss();
